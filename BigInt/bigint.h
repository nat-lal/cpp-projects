...
public:
  ...
  BigInt(int input,
         int base); // create BigInt from int (base 10) to a certain base
  BigInt(const string &s, int base); // create BigInt from string
  BigInt(
      const BigInt &b); // create BigInt from another BigInt (copy constructor)
  ~BigInt();            // destructor
  ...
  const BigInt &modulusExp(const BigInt &b,
                           const BigInt &mod); // mod exponentiation assignment
...
struct DivByZeroException : exception {
  const char *what() const throw() { return "Error: Divide by zero"; }
};

struct ExpByNegativeException : exception {
  const char *what() const throw() { return "Error: Exponentiate by negative"; }
};

struct DiffBaseException : exception {
  const char *what() const throw() { return "Error: Bases are not equal"; }
};
